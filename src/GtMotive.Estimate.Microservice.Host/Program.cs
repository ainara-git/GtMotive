...
// Changed to IsProduction, because Docker environment was created to ship the exercise, and key vault should not be used there.
...
builder.Services.Configure<MongoDbSettings>(builder.Configuration.GetSection("MongoDb"));   // Added MongoDB in Docker to persist data.
builder.Services.Configure<RabbitMqSettings>(builder.Configuration.GetSection("RabbitMq")); // Added RabbitMQ in Docker to publish domain events.
